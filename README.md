...
標籤會依答題狀況自動調整：新單字連續答對 2 次會升成 `known`；已經標成讀音或意思不確定的單字，連續答對 3 次才會升成 `known`。已會單字如果在讀音題答錯，會降成 `reading_unknown`，如果在意思題答錯，會降成 `meaning_unknown`。同一個單字出過後，至少隔 3 天才會再次被選為題目。

每日題目前，bot 會先送 10 個資料庫裡還沒有的新單字。你可以在同一個選單裡勾選「不會意思：單字」或「不會讀音：單字」；沒勾的會直接標成 `known`。不會意思的漢字詞也會一起排進讀音複習；純假名詞不會出現讀音選項。接著的 10 題會優先從剛剛標成不會意思的新單字出填空題；讀音題只會使用含漢字的單字。

手動呼叫 `!quiz` 或 `/quiz` 也會先進入新單字挑選流程。測驗中可以按「退出測驗」停止；答過的題目會保留在同一則訊息裡，錯題會顯示正確答案，以及你選錯的選項意思。
...
查字典時，bot 會先用本地日中詞典覆蓋表回傳中文；查不到中文時才查 Jisho 英文釋義並交給 Ollama 翻譯。若字典 entry 有例句，會優先直接使用字典例句；沒有例句時才請 Ollama 產生。意思題會以短句填空形式出題。
